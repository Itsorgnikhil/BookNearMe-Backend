...
import niketeck.StayNest.dto.GuestDto;
import niketeck.StayNest.dto.HotelReportDto;
import niketeck.StayNest.entity.*;
...
import niketeck.StayNest.repository.*;
import niketeck.StayNest.service.BookingService;
import niketeck.StayNest.service.CheckoutService;
import niketeck.StayNest.startergy.PricingService;
...
public class BookingServiceImpl implements BookingService{
    ...
    public BookingDto addGuests(Long bookingId, List<Long> guestIdList) {
        ...
        if (booking.getBookingStatus() != BookingStatus.RESERVED && booking.getBookingStatus() != BookingStatus.GUESTS_ADDED) {
            throw new IllegalStateException("Booking is not under reserved or guests added state, cannot add guests");
        }
    ...
    public void cancelBooking(Long bookingId) {
        ...
        if (originalStatus != BookingStatus.CONFIRMED && originalStatus != BookingStatus.RESERVED &&
            originalStatus != BookingStatus.GUESTS_ADDED && originalStatus != BookingStatus.PAYMENTS_PENDING) {
            throw new IllegalStateException("Only active or pending bookings can be cancelled");
        }
        ...
        if (originalStatus == BookingStatus.CONFIRMED) {
            ...
            // For pending statuses, release the reserved count rather than the booked count
            inventoryRepository.findAndLockReservedInventory(booking.getRoom().getId(), booking.getCheckInDate(),
                    booking.getCheckOutDate(), booking.getRoomsCount());
